<template>
        ...
        <div class="addCollectForm">
          ...
          <div class="content">
                <div class="collect_name">
                    <label for="">名称：</label>
                    <input type="text" placeholder="请输入收藏夹名称" v-model="collectionForm.collectionName">
                </div>
                <div class="collect_description">
                    <label for="">描述：</label>
                    <textarea type="text" placeholder="请输入收藏描述（限100字,选填）" rows="10" v-model="collectionForm.description">
                    </textarea>
  ...
  const collectionForm= reactive<Collection>({
  ...
  const submit=async()=>{
    //判断收藏夹名称为不为空
    if(collectionForm.collectionName ==undefined || collectionForm.collectionName==""){
        ElMessage({
            message:"收藏夹不为空",
            type:"warning"
        })
    ...
    //提交请求
   let res= await createCollection(collectionForm)
