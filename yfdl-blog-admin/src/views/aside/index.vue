<template>
    ...
    <el-sub-menu v-if="(parent.children && parent.children.length>0)" v-show="parent.meta.display=='0'" :index="parent.id.toString()">
        <template #title>
            <svg-icon  :icon-class="parent.meta.icon"/>
            <span class="title">{{parent.meta.title}}</span>
        </template>

        <el-menu-item :index="item.name" v-for="item in parent.children" v-show="item.meta.display=='0'">
            <svg-icon :icon-class="item.meta.icon"/>
            <template #title><span class="title">{{item.meta.title}}</span></template>
        </el-menu-item>
       
    </el-sub-menu>
    <el-menu-item v-else  :index="parent.name"  v-show="parent.meta.display=='0'">
        <svg-icon :icon-class="parent.meta.icon"/>
